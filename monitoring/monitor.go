...
func GetServerMetrics() database.ServerMetric {
	...
	var networkIn, networkOut, uploadRate, downloadRate float64
	//if len(netInfo) > 0 {
	//	networkIn = float64(netInfo[0].BytesRecv) / 1024 / 1024
	//	networkOut = float64(netInfo[0].BytesSent) / 1024 / 1024
	//}
	netStats, err := networkMonitor.GetCurrentStats()
	if err == nil {
		networkIn = float64(netStats.BytesRecv) / 1024 / 1024
		networkOut = float64(netStats.BytesSent) / 1024 / 1024
		uploadRate = netStats.UploadRate / 1024
		downloadRate = netStats.DownloadRate / 1024
	}
...
func ConvertToSystemMetrics(metric database.ServerMetric) SystemMetrics {
	return SystemMetrics{
		...
		UploadRate:       metric.UploadRate,
		DownloadRate:     metric.DownloadRate,
