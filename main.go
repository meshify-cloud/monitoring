...
func main() {
	...
	//}
	if token == "" {
		log.Fatal("token is required in the configuration")
	}
	...
	app.Get("/metrics", func(c *fiber.Ctx) error {
		...
		if limit == "all" {
			...
			for _, m := range dbMetrics {
				item := monitoring.ConvertToSystemMetrics(m)
				item.Bandwidth = cfg.Server.Bandwidth
